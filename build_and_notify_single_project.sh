...
for candidate in `git branch -a | sed -e 's/^..//' -e 's/ ->.*$//' -e 's,^remotes/,,' | sort -t / -k 2 -k 1 -k 3` ; do
	...
	if [ "$commit_time" -gt "$cutoff_time" ] ; then
		...
		if [ -z "$is_built" ] ; then
			...
			if [ x"$candidate" != "xmaster" ] ; then
				#Stick the branch name in the directory name, so if there is more than one branch being worked on, the
				#  user can tell which is which
				encoded_candidate="`echo \"${candidate}\" | perl -MURI::Escape -nle 'printf \"%s\", uri_escape($_)'`"
				build_identifier="${build_time_human}_${encoded_candidate}"
			else
			...
			if [ $? -ne 0 ] ; then
			  ...
			  cat "$OVER_AIR_INSTALLS_DIR/$target/$build_identifier/${build_target}_xcodebuild_stderr.txt" >&2
				echo "Build Failed" >&2
				echo "Extracting build error" >&2
				grep ' error: ' "$OVER_AIR_INSTALLS_DIR/$target/$build_identifier/${build_target}_xcodebuild_stdout.txt" > "$OVER_AIR_INSTALLS_DIR/$target/$build_identifier/${build_target}_build_errors.txt"
				if [ ! -s  "$OVER_AIR_INSTALLS_DIR/$target/$build_identifier/${build_target}_build_errors.txt" ] ; then
					#couldn't find any errors in output, add standard error
					cp "$OVER_AIR_INSTALLS_DIR/$target/$build_identifier/${build_target}_xcodebuild_stderr.txt" "$OVER_AIR_INSTALLS_DIR/$target/$build_identifier/${build_target}_build_errors.txt"
				fi
				ENCODED_ERROR=`perl -MURI::Escape -nle 'printf "%s%%0A", uri_escape($_)' "$OVER_AIR_INSTALLS_DIR/$target/$build_identifier/${build_target}_build_errors.txt"`
		  ...
		  cat "$OVER_AIR_INSTALLS_DIR/$target/$build_identifier/${build_target}_xcodebuild_stderr.txt" >&2
			...
			if [ -d "${App_location}" ] ; then
				xcrun -sdk iphoneos PackageApplication "${App_location}" -o "$OVER_AIR_INSTALLS_DIR/$target/tmp/$build_identifier/${App_name}.ipa" --sign "iPhone Developer" --embed "$provprofile"
			else
				echo "Could not find app '${App_name}' at relative location '${App_location}' full path '`pwd`/${App_location}'"
				rm -rf "$OVER_AIR_INSTALLS_DIR/$target/tmp/$build_identifier/${App_name}.ipa"
			...
			if [ ! -z "${XCAB_WEB_ROOT}" -a -f "/Applications/BetaBuilder for iOS Apps.app/Contents/MacOS/BetaBuilder for iOS Apps" ] ; then
				echo "running /Applications/BetaBuilder for iOS Apps.app/Contents/MacOS/BetaBuilder for iOS Apps" -ipaPath="$OVER_AIR_INSTALLS_DIR/$target/tmp/$build_identifier/${App_name}.ipa" -outputDirectory="$OVER_AIR_INSTALLS_DIR/$target/$build_time_human" -webserver="${XCAB_WEB_ROOT}/${target}/$build_time_human"
				
				"/Applications/BetaBuilder for iOS Apps.app/Contents/MacOS/BetaBuilder for iOS Apps" -ipaPath="$OVER_AIR_INSTALLS_DIR/$target/tmp/$build_identifier/${App_name}.ipa" -outputDirectory="$OVER_AIR_INSTALLS_DIR/$target/$build_time_human" -webserver="${XCAB_WEB_ROOT}/${target}/$build_time_human"
				if [ $? -ne 0 ] ; then
					rm -rf "$OVER_AIR_INSTALLS_DIR/$target/tmp/$build_identifier/${App_name}.ipa"
				...
				if [ -d "${App_location}.dSYM" ] ; then
					tar czf "$OVER_AIR_INSTALLS_DIR/$target/$build_identifier/${App_name}.app.dSYM.tar.gz" "${App_location}.dSYM"
				fi
				...
				if [ ! -z "$TESTFLIGHT_API_TOKEN" -a ! -z "$TESTFLIGHT_TEAM" ] ; then
					...
					if [ -z "`echo $ignore_opts | grep -i testflight`" ] ; then
					  curl http://testflightapp.com/api/builds.json -F file=@"$OVER_AIR_INSTALLS_DIR/$target/$build_identifier/${App_name}.ipa"  -F api_token="$TESTFLIGHT_API_TOKEN" -F team_token="$TESTFLIGHT_TEAM" -F notes='New ${target} Build was uploaded via the upload API' $TESTFLIGHT_NOTIFY 
				  fi
				fi
		    ...
		    if [ -z "`echo $ignore_opts | grep -i notify`" ] ; then
  				...
  				IPA_STATUS="`curl -sI -X HEAD \"${XCAB_WEB_ROOT}/${target}/$build_identifier/${App_name}.ipa\" | grep HTTP/1.1 | awk '{print $2}'`"
  				PLIST_STATUS="`curl -sI -X HEAD \"${XCAB_WEB_ROOT}/${target}/$build_identifier/manifest.plist\" | grep HTTP/1.1 | awk '{print $2}'`"
  				INDEX_STATUS="`curl -sI -X HEAD \"${XCAB_WEB_ROOT}/${target}/$build_identifier/index.html\" | grep HTTP/1.1 | awk '{print $2}'`"
  				RETRY_COUNT=0
  				while [ "$IPA_STATUS" != "200"  -o "$PLIST_STATUS" != "200"  -o "$INDEX_STATUS" != "200" ] ; do
  					...
  					if [ "$RETRY_COUNT" -gt 30 ] ; then
  						...
  						$my_dir/notify_with_boxcar.sh "notification[source_url]=${XCAB_WEB_ROOT}/$target/$build_identifier/index.html" "notification[message]=ERROR+Timeout+Waiting+For+Webserver+For+${target}+Build"
  						echo "$sha" > "$OVER_AIR_INSTALLS_DIR/$target/$build_identifier/sha.txt" #Don't try to build this again - it would generate timeouts over and over if the web server is down
							rm -rf "$OVER_AIR_INSTALLS_DIR/$target/tmp/$build_identifier/"
							exit 4
  					fi
  				done
  				...
  				#Notify with Boxcar
  				$my_dir/notify_with_boxcar.sh "notification[source_url]=${XCAB_WEB_ROOT}/$target/$build_identifier/index.html" "notification[message]=New+${target}+Build+available"
  		  fi
			else
				#Put in the permenant location
				cp "$OVER_AIR_INSTALLS_DIR/$target/tmp/$build_identifier/${App_name}.ipa" "$OVER_AIR_INSTALLS_DIR/$target/$build_identifier/${App_name}.ipa"
				
				#Save off the symbols, too
				if [ -d "${App_location}.dSYM" ] ; then
					tar czf "$OVER_AIR_INSTALLS_DIR/$target/$build_identifier/${build_target}.app.dSYM.tar.gz" "${App_location}.dSYM"
				fi
				...
				if [ ! -z "$TESTFLIGHT_API_TOKEN" -a ! -z "$TESTFLIGHT_TEAM" ] ; then
					...
					echo "ipa saved to $OVER_AIR_INSTALLS_DIR/$target/$build_identifier/" >&2
			...
			echo "$sha" > "$OVER_AIR_INSTALLS_DIR/$target/$build_identifier/sha.txt"
			rm -rf "$OVER_AIR_INSTALLS_DIR/$target/tmp/$build_identifier/"
		fi
	fi
done
